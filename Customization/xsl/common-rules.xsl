...
<xsl:stylesheet xmlns:xhtml="http://www.w3.org/1999/xhtml"
	...
	<xsl:template match="section|topic" mode="common-mandatory-id">
		...
		<xsl:if test="not(@id)">
			<xsl:call-template name="fired-rule"/>
			<xsl:call-template name="failed-assert">
				<xsl:with-param name="rule-id"><xsl:value-of select="name()" />-id-missing</xsl:with-param>
				<xsl:with-param name="test">(name() = '<xsl:value-of select="name()" />') and not(@id)</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="*[@conref]" mode="conref-structure-rules">
		...
		<xsl:if test="not($isFileRefAndFileExists) and $idRef">
			<xsl:choose>
				...
				<!--
					conref-internal-path-mismatch  - For a conref to within a single file, the path linked to must be
																				a node of the same type as the original
				-->
		...
		<xsl:if test="$isFileRefAndFileExists and $idRef">
			<xsl:choose>
				...
				<!--
					conref-external-id-mismatch - For a conref to an another file within the document, the ID linked to must be
																				a node of the same type as the original
				-->
				<xsl:when test="$idRefNode and not($idIdRefNode) and not(name() = $idRefNode/name())">
					<xsl:call-template name="failed-assert">
				...
				<!--
					conref-external-path-mismatch - For a conref to an another file within the document, the path linked to must be
																				a node of the same type as the original
				-->
				<xsl:when test="$idIdRefNode and ($idIdRefNode/ancestor:: */@id=$idRef)  and not(name() = $idIdRefNode/name())">
					<xsl:call-template name="failed-assert">
